...
const getAgencyInfoByUser = async (uid) => {
    try {
        const getRef = await AgencyRefModel.findOne({
            where: { uid },
            attributes: ["uid", "agency"]
        });
        if (!getRef) return null;
        const getRefAgencyInfo = await AgencyModel.findOne({
            where: { id: getRef.agency },
            attributes: ["uid", "code"]
        });
        if (!getRefAgencyInfo) return null;
        const getRefAgencyUserInfo = await UserModel.findOne({
            where: { id: getRefAgencyInfo.uid },
            attributes: { exclude: ["password", "deletedAt", "code", "status", "updatedAt"] },
            raw: true,
            nest: true,                      
        });
        if (!getRefAgencyUserInfo) return null;
        const copiedObject = {
            ...getRefAgencyUserInfo,
            agency: {
                ref: {
                    uid: getRef.uid,
                    agency: getRef.agency
                },
                code: getRefAgencyInfo.code
            }
        };
        return copiedObject;
    } catch (e) {
