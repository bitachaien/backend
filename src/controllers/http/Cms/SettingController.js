...
const { ConfigModel } = require("@Models/Configs/Configs");
...
module.exports = {
    ...
    General: async (req, res) => {
        try {
            const getConfigSite = await ConfigModel.getConfigFromDB(ConfigModel.KEY_ENUM.SITE_INFOMATION);
            res.status(200).json({
                status: true,
    ...
    Action: {
        updateGeneral: async (req, res) => {
            try {
                ...
                if (!await ConfigModel.setConfigFromDB(ConfigModel.KEY_ENUM.SITE_INFOMATION, currentFileSetting)) {
        ...
        updateTelegram: async (req, res) => {
            try {
                ...
                if (!await ConfigModel.setConfigFromDB(ConfigModel.KEY_ENUM.TELEGRAM_BOT, getJsonConfig(`${_SRC_PATH}/configs/telegram/bot.json`))) {
                    return res.status(200).json({
                        status: false,
                        msg: ERROR_SERVER.WRONG,
                        code: 500
                ...
                if (!await ConfigModel.setConfigFromDB(ConfigModel.KEY_ENUM.TELEGRAM_CHAT_GROUP, getJsonConfig(`${_SRC_PATH}/configs/telegram/chatGroup.json`))) {
                    return res.status(200).json({
                        status: false,
                        msg: ERROR_SERVER.WRONG,
                        code: 500
                ...
                if (!await ConfigModel.setConfigFromDB(ConfigModel.KEY_ENUM.TELEGRAM_MESSAGE, getJsonConfig(`${_SRC_PATH}/configs/telegram/message.json`))) {
                    return res.status(200).json({
                        status: false,
                        msg: ERROR_SERVER.WRONG,
                        code: 500
        ...
        TelegramTest: async (req, res) => {
            try {
                ...
                switch (type) {
                    case "depositCard":
                        response = await teleBotSendMsg(chatTeleConfig.paymentCard, messageTeleConfig.paymentCard, {
                            '{{time}}': moment().format("DD/MM/YYYY HH:MM:ss"),
                            '{{username}}': "Test Username",
                            '{{name}}': "Test Name",
                            '{{phone}}': "0123456788",
                            '{{email}}': "Test@78968.site",
                            '{{amount}}': Helper.numberWithCommas(500000),
                            '{{balance}}': Helper.numberWithCommas(230000),
                            '{{transId}}': "1234321",
                            '{{network}}': "VIETTEL",
                            '{{pin}}': "9372957395345",
                            '{{seri}}': "564345893453543"
                        });
    
                        break;
    
                    case "depositBank":
                        response = await teleBotSendMsg(chatTeleConfig.paymentBank, messageTeleConfig.paymentBank, {
                            '{{time}}': moment().format("DD/MM/YYYY HH:MM:ss"),
                            '{{username}}': "Test Username",
                            '{{name}}': "Test Name",
                            '{{phone}}': "0123456788",
                            ...
                            '{{balance}}': Helper.numberWithCommas(230000),
                            '{{transId}}': "1234321",
                            '{{chargeTypeProvide}}': "VIETCOMBANK",
                            '{{chargeTypeProvideVi}}': "Ngân hàng"
                        });
                        break;
    
                    case "WithdrawBank":
                        response = await teleBotSendMsg(chatTeleConfig.paymentWithdraw, messageTeleConfig.paymentWithdraw, {
                          '{{time}}': moment().format("DD/MM/YYYY HH:MM:ss"),
                          '{{username}}': "Test Username",
                          '{{name}}':"Test Name",
                          '{{phone}}': "0123456788",
