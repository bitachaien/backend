    ...
    listDeposit: async (req, res) => {
        try {
            ...
            if (!!page && !!kmess) {
                ...
                Task.push(await CardHistoryModel.count({ where: match }));
                Task.push(await CardHistoryModel.findAll({
                    where: match,
                ...
                getDataCount.map((row) => {
                    ...
                    if (row.status == CardHistoryModel.STATUS_ENUM.SUCCESS) totalSuccess += row.amount;
                    if (row.status == CardHistoryModel.STATUS_ENUM.PENDING) totalPending += row.amount;
                    if (row.status == CardHistoryModel.STATUS_ENUM.ERROR) totalFail += row.amount;
                });
