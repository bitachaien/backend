  ...
  listDeposit: async (req, res) => {
    try {
      ...
      if (!!page && !!kmess) {
        ...
        Task.push(await BankHistoryModel.scope(assocScopes).count({ where: match }));
        Task.push(await BankHistoryModel.scope(assocScopes).findAll({
          where: match,
        ...
        let totalAmount = totalSuccess = totalFail = 0;
        getDataCount.map((row) => {
          totalAmount += row.amount;
          if (row.status == BankHistoryModel.STATUS_ENUM.SUCCESS) totalSuccess += row.amount;
          if (
            row.status == BankHistoryModel.STATUS_ENUM.TIMEOUT ||
            row.status == BankHistoryModel.STATUS_ENUM.ERROR) totalFail += row.amount;
        });
